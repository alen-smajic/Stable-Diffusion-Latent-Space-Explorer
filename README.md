...
Each experiment run will produce a results folder, which will be used for storing the generated results. This results folder will have a unique name consisting of the date and time of execution, the model identifier and the experiment identifier. Furthermore, the results folder may contain several subfolders including:
 ...
 - ```embeddings``` stores the unique latents for each generated image.
  - ```gifs``` stores the gifs generated by stacking the frames from the ```images``` folder. 
